...
//sigue
    ...
    while($control == 0 ){ 
        ...
        if ($inicio== 0){
            $url = "https://4572765.restlets.api.netsuite.com/app/site/hosting/restlet.nl?script=1896&deploy=1&start=$start&end=$end";
        }
        ...
        if ($control == 0){
            ...
            for($i=0; $i<$longitud; $i++){ 
                ...
                $id                         = $data[$i]["id"];
                $ubicacion_id               = empty($data[$i]["values"]["CUSTRECORD_AXA_ZDV_LISTA_SEGMENTO_PJCL.internalid"][$y]["text"]) ? 0 : $data[$i]["values"]["CUSTRECORD_AXA_ZDV_LISTA_SEGMENTO_PJCL.internalid"][$y]["text"];
                $lista_id                   = empty($data[$i]["values"]["CUSTRECORD_AXA_ZDV_UNIDAD_NEGOCIO_PJCL.internalid"][$y]["text"]) ? 0 : $data[$i]["values"]["CUSTRECORD_AXA_ZDV_UNIDAD_NEGOCIO_PJCL.internalid"][$y]["text"];
                $responsable_id             = empty($data[$i]["values"]["CUSTRECORDAXA_RESZDV_.internalid"][$y]["text"]) ? 0 : $data[$i]["values"]["CUSTRECORDAXA_RESZDV_.internalid"][$y]["text"];
                $clase_id                   = empty($data[$i]["values"]["CUSTRECORD_AXA_ZDV_CLASE_PJCL.internalid"][$y]["text"]) ? 0 : $data[$i]["values"]["CUSTRECORD_AXA_ZDV_CLASE_PJCL.internalid"][$y]["text"];
                $nombre                     = $data[$i]["values"]["name"];
                $isinactive                 = $data[$i]["values"]["isinactive"];
                ...
                $sql1_validar = "SELECT id FROM zona WHERE id = '".$id."'";
                $result1 = pg_query( $dbconn, $sql1_validar );
                if (pg_num_rows($result1) > 0){

                    //actualiza
                    $sql1 = "UPDATE zona SET 
                        id = $id, 
                    ...
                    //creación
                    $sql1 = "INSERT INTO zona 
                    (id, ubicacion_id, lista_id, responsable_id, clase_id, nombre, isinactive, created_at) 
                    VALUES ($id, $ubicacion_id, $lista_id, $responsable_id, $clase_id, '$nombre', '$isinactive', CURRENT_TIMESTAMP)";
    ...
    $sql_log = "INSERT INTO log (proceso, tabla, cant_registro, cant_insertados, 
        ...
        ('REGISTRO', 'zona', $cantidad_registros, $valores_creados, $valores_actualizados, 
