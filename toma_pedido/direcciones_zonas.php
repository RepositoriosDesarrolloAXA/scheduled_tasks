...
/* --------------------------------------------- Insertar Direcciones de las zonas --------------------------------------------- */
...
//sigue
    ...
    while($control == 0 ){ 
        ...
        if ($inicio== 0){
            $url = "https://4572765.restlets.api.netsuite.com/app/site/hosting/restlet.nl?script=1898&deploy=1&start=$start&end=$end";
        }
        ...
        if ($control == 0){
            ...
            for($i=0; $i<$longitud; $i++){ 
                ...
                $direccion_id                   = $data[$i]["values"]["GROUP(Address.addressinternalid)"];
                $zonas_id                       = $data[$i]["values"]["GROUP(formulatext)"];

                $separar_zonas = explode(",", $zonas_id);

                foreach ( $separar_zonas as $separado ) {
                    $zonas[] = $separado;
                }
    ...
    if(count($data_info) > 0){
        foreach ($data_info as $key => $value) {
            ...
            $direccion_id = $value["direccion_id"];
            $zona_id = $value["zona_id"];
            
            $sql1_validar = "SELECT id FROM direccion_zona WHERE direccion_id = '".$direccion_id."' AND zona_id = '".$zona_id."'";
            //print($sql1_validar);
            $result1 = pg_query( $dbconn, $sql1_validar );
            if (pg_num_rows($result1) > 0){

                //actualiza
                $sql1 = "UPDATE direccion_zona SET 
                    direccion_id  = $direccion_id, 
                    zona_id = $zona_id, 
                    updated_at = CURRENT_TIMESTAMP 
                    WHERE direccion_id = '".$direccion_id."' AND zona_id = '".$zona_id."'";
                ...
                //creación
                $sql1 = "INSERT INTO direccion_zona 
                (direccion_id, zona_id, created_at) 
                VALUES ($direccion_id, $zona_id, CURRENT_TIMESTAMP)";
    ...
    $sql_log = "INSERT INTO log (proceso, tabla, cant_registro, cant_insertados, 
        ...
        ('REGISTRO', 'direccion_zona', $cantidad_registros, $valores_creados, $valores_actualizados, 
