...
//sigue
    ...
    while($control == 0 ){ 
        ...
        if ($inicio== 0){
            $url = "https://4572765.restlets.api.netsuite.com/app/site/hosting/restlet.nl?script=1899&deploy=1&start=$start&end=$end";
        }
        ...
        if ($control == 0){
            ...
            for($i=0; $i<$longitud; $i++){ 
                ...
                $user_id            = $data[$i]["values"]["CUSTRECORD_AXA_ZDV_ASESOR_COMERCIAL_PJCL.internalid"][$y]["text"];
                $zona_id            = $data[$i]["values"]["internalid"][$y]["text"];
                ...
                */
                ...
                $sql1_validar = "SELECT * FROM zona_asesor WHERE user_id = '".$user_id."' AND zona_id = '".$zona_id."'";
                //print($sql1_validar);
                $result1 = pg_query( $dbconn, $sql1_validar );
                if (pg_num_rows($result1) > 0){

                    //actualiza
                    $sql1 = "UPDATE zona_asesor SET 
                        user_id = $user_id, 
                        zona_id = '$zona_id', 
                        updated_at = CURRENT_TIMESTAMP
                        WHERE user_id = '".$user_id."' AND zona_id = '".$zona_id."'";
                    ...
                    //creación
                    $sql1 = "INSERT INTO zona_asesor 
                    (user_id, zona_id, created_at) 
                    VALUES ($user_id, $zona_id, CURRENT_TIMESTAMP)";
    ...
    $sql_log = "INSERT INTO log (proceso, tabla, cant_registro, cant_insertados, 
        ...
        ('REGISTRO', 'zona_asesor', $cantidad_registros, $valores_creados, $valores_actualizados, 
