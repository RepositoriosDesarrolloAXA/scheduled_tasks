...
//sigue
    ...
    while($control == 0 ){ 
        ...
        if ($inicio== 0){
            $url = "https://4572765.restlets.api.netsuite.com/app/site/hosting/restlet.nl?script=2011&deploy=1&start=$start&end=$end";
        }
        ...
        if ($control == 0){
            ...
            for($i=0; $i<$longitud; $i++){ 
                ...
                $item_id                     = empty($data[$i]["values"]["CUSTRECORD_AXA_ARTICULOCANTIDAD.internalid"][$y]["text"]) ? 'null' : $data[$i]["values"]["CUSTRECORD_AXA_ARTICULOCANTIDAD.internalid"][$y]["text"];
                $ubicacion_id                = empty($data[$i]["values"]["CUSTRECORD_AXA_UBICACIONCANTID.internalid"][$y]["text"]) ? 'null' : $data[$i]["values"]["CUSTRECORD_AXA_UBICACIONCANTID.internalid"][$y]["text"];
                $cantidad                    = empty($data[$i]["values"]["custrecord_axa_cantidadmaxima"]) ? 0 : $data[$i]["values"]["custrecord_axa_cantidadmaxima"];
		...
		//alidar si existe
                ...
                $sql1_validar = "SELECT id FROM cantidad_maxima WHERE item_id = '".$item_id."' AND ubicacion_id = '".$ubicacion_id."'";
                $result1 = pg_query( $dbconn, $sql1_validar );
                if (pg_num_rows($result1) > 0){

                    //actualiza
                    $sql1 = "UPDATE cantidad_maxima SET  
                        item_id = $item_id, 
                        ubicacion_id = $ubicacion_id,  
						cantidad = $cantidad,  
                        updated_at = '$fecha_insertar'
                        WHERE item_id = '".$item_id."' AND ubicacion_id = '".$ubicacion_id."'";
                    ...
                    //creación
                    $sql1 = "INSERT INTO cantidad_maxima 
                    (item_id, ubicacion_id, cantidad, created_at) 
                    VALUES ($item_id, $ubicacion_id, $cantidad, '$fecha_insertar')";
    ...
    $sql_log = "INSERT INTO log (proceso, tabla, cant_registro, cant_insertados, 
        ...
        ('REGISTRO', 'cantidad_maxima', $cantidad_registros, $valores_creados, $valores_actualizados, 
