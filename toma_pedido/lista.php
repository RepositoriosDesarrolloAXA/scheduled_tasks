...
//sigue
    ...
    while($control == 0 ){ 
        ...
        if ($inicio== 0){
            $url = "https://4572765.restlets.api.netsuite.com/app/site/hosting/restlet.nl?script=1901&deploy=1&start=$start&end=$end";
        }
        ...
        if ($control == 0){
            ...
            for($i=0; $i<$longitud; $i++){ 
                ...
                $tipo_lista                 = $data[$i]["values"]["custrecord_axa_tipo_list"][$y]["text"];
                $inactivo                   = $data[$i]["values"]["formulatext"];
                $margen_credito             = $data[$i]["values"]["custrecord145"];
                $margen_contado             = $data[$i]["values"]["custrecord_nso_margen_2"];
                $mp_credito                 = $data[$i]["values"]["formulatext_2"];
                $mp_contado                 = $data[$i]["values"]["formulatext_1"];
                ...
                if(empty($margen_credito)){
                    $margen_credito = 0;
                }

                if(empty($margen_contado)){
                    $margen_contado = 0;
                }
            	...
            	if(!empty($id)){
                	...
                	$sql1_validar = "SELECT id FROM lista WHERE id = '".$id."'";
                	$result1 = pg_query( $dbconn, $sql1_validar );
                	if (pg_num_rows($result1) > 0){

                    	//actualiza
                    	$sql1 = "UPDATE lista SET 
                        	id = $id, 
                        	nombre = '$nombre', 
                    	...
                    	//creación
                    	$sql1 = "INSERT INTO lista 
                    	(id, nombre, tipo_lista, inactivo, margen_credito, margen_contado, mp_credito, mp_contado, created_at) 
                    	VALUES ($id, '$nombre', '$tipo_lista', '$inactivo', $margen_credito, $margen_contado, 
                    	$mp_credito, $mp_contado, '$fecha_insertar')";
    ...
    $sql_log = "INSERT INTO log (proceso, tabla, cant_registro, cant_insertados, 
        ...
        ('REGISTRO', 'lista', $cantidad_registros, $valores_creados, $valores_actualizados, 
