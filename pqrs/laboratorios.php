    ...
    for ($i=0; $i < $cantidad_dividir ; $i++) {
        ...
        if(!empty($data)){
            for($a=0; $a<count($data); $a++){
                ...
                $internal_id                = empty($data[$a]["values"]["GROUP(CUSTITEM_AXA_VENDORMANUFACTURER_PJCL.internalid)"][$y]) ? $data[$a]["values"]["GROUP(CUSTITEM_AXA_VENDORMANUFACTURER_PJCL.internalid)"][$y]["text"] : '';
