...
//sigue
    ...
    while($control == 0 ){ 
        ...
        if ($inicio== 0){
            $url = "https://4572765.restlets.api.netsuite.com/app/site/hosting/restlet.nl?script=2089&deploy=1&start=$start&end=$end";
        }
        ...
        if ($control == 0){
            ...
            for($i=0; $i<$longitud; $i++){ 
                ...
                $internal_id                    = $data[$i]["values"]["internalid"][$y]["text"];
                $numero                         = $data[$i]["values"]["tranid"];
                $fecha_creacion                 = $data[$i]["values"]["datecreated"];
                $nit_cliente                    = $data[$i]["values"]["entity"][$y]["text"];
                $nombre_cliente                 = str_replace("'", "", $data[$i]["values"]["custbody_axa_field_nomcliente_fjsr"]);
                $direccion_cliente              = str_replace("'", "", $data[$i]["values"]["shipaddress"]);
                $estado                         = $data[$i]["values"]["statusref"][$y]["text"];
                $valor                          = $data[$i]["values"]["amount"];
                $concepto_cliente               = $data[$i]["values"]["custbody_axa_camp_concepcliente"][$y]["text"];
                $factura_aplicada               = $data[$i]["values"]["appliedtotransaction"][$y]["text"];
    ...
    $sql_log = "INSERT INTO log (proceso, tabla, cant_registro, cant_insertados, 
        ...
        ('REGISTRO', 'Notas Creditos', $cantidad_registros, $valores_creados, $valores_actualizados, 
