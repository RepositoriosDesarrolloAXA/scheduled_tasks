<?php
/*
	Relacionar los pagos con el diario recien creado
*/

function cruzarPayWithJournal($dbconn, $payment){
	try{
    	// Armar body
		$body = new stdClass();
		$body->pago_id = $payment->id_interno;
    	$body->diario_id = $payment->id_journal;
    	$body->valor_pago_no_aplicado = $payment->monto_aplicado;
    	print('Cruxando Pay con Diario' . json_encode($body, true)  . "\n");
    	$responseNetSuite = enviar_a_netsuite($body);
    	...
    	$sql_log = "
        	INSERT INTO logs (
            	...
            	'Cruce de Pay a favor con Diario: " . json_encode($body, true) . "', 'saldos_inf_a_favor', 'Cruce de Pago con Diario: {$responseNetSuite}', 1, 1, 1, '$fecha_LOG', null, null, null, '$fecha_insertar'
            )
        ";
    	...
    	if($decodificado['message'] === 'success'){
        	...
        	$sqlUpdateRow = "
            	...
            	WHERE id_interno=$payment->id_interno AND id_journal=$payment->id_journal AND monto_aplicado=$payment->monto_aplicado
            ";
        	$resultUpdate = pg_query($dbconn, $sqlUpdateRow);
        	if(!$resultUpdate){
            	print("$payment->id_interno Falló al actualizar \n");
            }else{
            	print("$payment->id_interno Actualizado con exito \n");
            }
        }else{
            print('Error en NetSuite al intentar cruzar con el Diario: ' . $decodificado['response'] . "\n");
        }
    }catch(\Exception $err){
    	print("Error catch, Ocurrió un error al cruzar pago con Diario: $err");
    };
}
